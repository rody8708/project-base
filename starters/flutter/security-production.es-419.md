...
```text
HttpTaskRepository(apiUrl, tokenProvider: () => sessionToken)
```
...
La prueba HTTP usa FOUNDATION_TEST_API_URL y FOUNDATION_TEST_TOKEN_FILE (ruta a JSON privado con token). El flujo Windows usa --dart-define=API_BASE_URL y --dart-define=API_TEST_TOKEN_FILE: se incorpora solo una ruta de prueba, nunca el secreto. Este flujo es exclusivamente de integración, no distribución.
