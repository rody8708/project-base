...
Manual CI prepares web/maintenance (React and native web), Flutter per platform, Kotlin Android, and PHP/SQLite. YAML was parsed, including the new `web-vanilla` installation/check steps; it retains four job families and manual-only triggering. No remote repository was created and no jobs were triggered. Real execution is required before attributing results to the YAML.
