...
In `src/config.js`, set `API_BASE_URL` to `http://127.0.0.1:8080/api/v1`. Then, in PowerShell:
...
The origin enables only that connection in the local server CSP. The backend must allow `http://127.0.0.1:5180` in `API_ALLOWED_ORIGINS`. Restart after edits: the server serves a snapshot. `API_BASE_URL = ''` selects memory. Deployment needs its own CSP policy.
