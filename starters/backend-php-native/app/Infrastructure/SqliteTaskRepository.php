...
/** SQLite only: other engines need their own verified adapter and migrations. */
final readonly class SqliteTaskRepository implements TaskRepository
{
    public function __construct(private PDO $database, private IdentityContext $identity)
    {
        if ($database->getAttribute(PDO::ATTR_DRIVER_NAME) !== 'sqlite') {
            throw new \InvalidArgumentException('SQLite connection required.');
        }
