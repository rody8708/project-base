...
use App\Infrastructure\SqliteTokenAuthenticator;
use App\Http\Api;
...
try {
    ...
    $api = new Api(new TaskService(new SqliteTaskRepository($database, $identity), $newId),
        new SqliteTokenAuthenticator($database, static fn (): int => time()), $identity,
        new SqliteRateLimiter($database, static fn (): int => time()),
