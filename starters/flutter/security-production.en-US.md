...
The HTTP test uses FOUNDATION_TEST_API_URL and FOUNDATION_TEST_TOKEN_FILE (path to private token JSON). The Windows flow uses --dart-define=API_BASE_URL and --dart-define=API_TEST_TOKEN_FILE: only a test path is embedded, never the secret. This is an integration-only flow, not distribution.
