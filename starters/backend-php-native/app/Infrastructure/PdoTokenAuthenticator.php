...
readonly class PdoTokenAuthenticator implements TokenAuthenticator, TokenStore
{
    public function __construct(private PDO $database, private \Closure $now)
    {
        if (!in_array($database->getAttribute(PDO::ATTR_DRIVER_NAME), ['sqlite', 'pgsql', 'mysql'], true)) throw new \InvalidArgumentException('Supported SQL required.');
