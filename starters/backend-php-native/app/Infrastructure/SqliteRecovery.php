...
final class SqliteRecovery
{
    ...
    public function snapshot(string $source, string $destination): array
    {
        ...
        $snapshot = $db->prepare('VACUUM INTO ?');
        $snapshot->execute([$backup]);
        $snapshot = null;
        $db = null;
        chmod($backup, 0600);
    ...
    public function restore(string $directory, string $destination): array
    {
        ...
        $db->beginTransaction();
        try {
            $invalidate = $db->prepare('UPDATE api_tokens SET revoked_at = ?');
            $invalidate->execute([time()]);
            $revoked = $invalidate->rowCount();
