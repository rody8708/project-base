...
[US English](LICENSE-SCOPE.en-US.md) · [Inicio](README.es-419.md) · [Texto oficial MPL-2.0](LICENSE)
...
Las dependencias, herramientas, binarios generados, wrappers y otros materiales de terceros conservan sus propias licencias y avisos. La MPL-2.0 de este proyecto no cambia esos términos. En particular, `GRADLE-LICENSE.txt` corresponde a Gradle; los gestores de paquetes y locks identifican otras dependencias que deben revisarse al distribuir un producto.

Los artefactos inmutables conservados dentro de `releases/` son registros históricos y no se modificaron al adoptar esta licencia. Para una publicación nueva se debe generar un artefacto que incluya `LICENSE` y sus avisos aplicables.

MPL-2.0 no concede derechos sobre marcas. Consulta la [política de marcas](TRADEMARKS.es-419.md). Este documento explica la intención del proyecto y no sustituye el texto de la licencia ni asesoría jurídica para un caso concreto.
