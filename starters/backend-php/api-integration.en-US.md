...
Prepare a new copy following the README. Before starting the local server:
...
Without configured origins, cross-origin browser access is not enabled. The API does not depend on the frontend: it generates no screens, imports no client files, and continues persisting through `TaskRepository`/`SqlTaskRepository`. Select the engine in configuration and run migrations only against an authorized target. `composer check` checks local contract/HTTP/persistence; `php -d extension=pdo_pgsql scripts/qa-databases.php --wsl-docker` repeats the suite against all three isolated engines.
